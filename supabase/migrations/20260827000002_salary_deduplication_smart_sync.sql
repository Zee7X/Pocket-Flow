-- =============================================================================
-- Migration: Smart salary transaction deduplication & sync in pf_allocate_salary
-- =============================================================================
...
BEGIN
    ...
    IF FOUND THEN
        ...
        -- Update corresponding auto-generated income transaction if exists
        UPDATE public.pf_transactions
        SET amount = p_salary_amount,
            transaction_date = p_salary_date
        WHERE user_id = v_user_id
          AND (note = 'Gaji Periode ' || p_period_month || '/' || p_period_year
               OR (type = 'income' AND EXTRACT(MONTH FROM transaction_date) = p_period_month AND EXTRACT(YEAR FROM transaction_date) = p_period_year AND note ILIKE '%gaji%'));
    ...
    ELSE
        ...
        -- 2. Check if user already manually entered an income transaction with 'gaji' in this month
        SELECT id INTO v_existing_tx_id
        FROM public.pf_transactions
        WHERE user_id = v_user_id
          AND type = 'income'
          AND (note ILIKE '%gaji%' OR (v_income_cat_id IS NOT NULL AND category_id = v_income_cat_id))
    ...
    LOOP
        ...
        ELSIF v_rule.allocation_type = 'percentage' THEN
            IF v_rule.percentage IS NOT NULL AND v_rule.percentage > 0 THEN
                IF v_rule.percentage_base = 'baseline' AND v_baseline_income > 0 THEN
                    v_pct_base := v_baseline_income;
                ELSIF v_rule.percentage_base = 'extra' THEN
                    v_pct_base := v_extra_income;
                ELSIF v_rule.percentage_base = 'remaining' THEN
