-- =============================================================================
-- Migration: Fix pf_allocate_salary & pf_preview_allocation deficit & shortfall detection
-- =============================================================================
...
BEGIN
    ...
    LOOP
        ...
        IF v_rule.allocation_type = 'fixed' THEN
            v_expected_amount := COALESCE(v_rule.fixed_amount, 0);
            v_allocation_amount := v_expected_amount;
            IF NOT v_allow_overalloc THEN
                IF v_rule.is_required AND v_remaining < v_expected_amount THEN
                    v_warnings := v_warnings || jsonb_build_object(
                        'code',      CASE WHEN v_remaining <= 0 THEN 'INSUFFICIENT_FUNDS' ELSE 'SHORTFALL' END,
                        'rule_id',   v_rule.id,
                        'rule_name', v_rule.name,
                        'shortfall', GREATEST(v_expected_amount - v_remaining, 0),
                        'message',   'Gaji tidak mencukupi untuk pos wajib: ' || v_rule.name
                    );
                END IF;
        ...
        ELSIF v_rule.allocation_type = 'percentage' THEN
            IF v_rule.percentage IS NOT NULL AND v_rule.percentage > 0 THEN
                ...
                IF NOT v_allow_overalloc THEN
                    IF v_rule.is_required AND v_remaining < v_allocation_amount THEN
                        v_warnings := v_warnings || jsonb_build_object(
                            'code',      CASE WHEN v_remaining <= 0 THEN 'INSUFFICIENT_FUNDS' ELSE 'SHORTFALL' END,
                            'rule_id',   v_rule.id,
                            'rule_name', v_rule.name,
                            'shortfall', GREATEST(v_allocation_amount - v_remaining, 0),
                            'message',   'Gaji tidak mencukupi untuk pos wajib: ' || v_rule.name
                        );
                    END IF;
                    v_allocation_amount := LEAST(v_allocation_amount, v_remaining);
...
'PocketFlow: Read-only preview of allocation with accurate shortfall vs remainder warning categorization.';
...
'PocketFlow: Core allocation engine with accurate shortfall vs unallocated remainder warning handling.';
